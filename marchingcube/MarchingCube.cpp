...
Point3 LinearInterp(mp4vector p1, mp4vector p2, float value)
{
	Point3 p;
	if (fabs(p1.val - p2.val) > 0.00001)
		p = (Point3)p1 + ((Point3)p2 - (Point3)p1) / (p2.val - p1.val)*(value - p1.val);
	else
		p = (Point3)p1;
	return p;
}
...
TRIANGLE* MarchingCubes(int ncellsX, int ncellsY, int ncellsZ,
	...
	float minValue, std::vector<mp4vector> points, int &numTriangles)
{
	...
	Point3 intVerts[12];			    //linearly interpolated vertices on each edge
	...
	Point3 grads[12];				    //linearly interpolated gradients on each edge
	...
	Point3 factor(1.0 / (2.0*gradFactorX), 1.0 / (2.0*gradFactorY), 1.0 / (2.0*gradFactorZ));
		...
		for (int j = 0; j < lastY; j++) 		//y axis
		{
			...
			for (int k = 0; k < lastZ; k++, ind++)	//z axis
			{
				...
				if (edgeIndex & 1) {
					...
					grads[0].data()[0] *= factor.data()[0]; grads[0].data()[1] *= factor.data()[1]; grads[0].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 2) {
					...
					grads[1].data()[0] *= factor.data()[0]; grads[1].data()[1] *= factor.data()[1]; grads[1].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 4) {
					...
					grads[2].data()[0] *= factor.data()[0]; grads[2].data()[1] *= factor.data()[1]; grads[2].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 8) {
					...
					grads[3].data()[0] *= factor.data()[0]; grads[3].data()[1] *= factor.data()[1]; grads[3].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 16) {
					...
					grads[4].data()[0] *= factor.data()[0]; grads[4].data()[1] *= factor.data()[1]; grads[4].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 32) {
					...
					grads[5].data()[0] *= factor.data()[0]; grads[5].data()[1] *= factor.data()[1]; grads[5].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 64) {
					...
					grads[6].data()[0] *= factor.data()[0]; grads[6].data()[1] *= factor.data()[1]; grads[6].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 128) {
					...
					grads[7].data()[0] *= factor.data()[0]; grads[7].data()[1] *= factor.data()[1]; grads[7].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 256) {
					...
					grads[8].data()[0] *= factor.data()[0]; grads[8].data()[1] *= factor.data()[1]; grads[8].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 512) {
					...
					grads[9].data()[0] *= factor.data()[0]; grads[9].data()[1] *= factor.data()[1]; grads[9].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 1024) {
					...
					grads[10].data()[0] *= factor.data()[0]; grads[10].data()[1] *= factor.data()[1]; grads[10].data()[2] *= factor.data()[2];
				}
				if (edgeIndex & 2048) {
					...
					grads[11].data()[0] *= factor.data()[0]; grads[11].data()[1] *= factor.data()[1]; grads[11].data()[2] *= factor.data()[2];
				}
	...
	float minValue, mp4vector * points, int &numTriangles)
{
	...
	Point3 prevIntVerts[4];		//passes interpolated vertices on edges
	mp4vector prevGradVerts[4];		//passes gradients at vertices
	Point3 prevGrads[4];			//passes interpolated gradients on edges
...
TRIANGLE* MCFace0(int ncellsX, int ncellsY, int ncellsZ,
	...
	mp4vector prevVerts[4], Point3 prevIntVerts[4], int edgeIndex,
	mp4vector prevGradVerts[4], Point3 prevGrads[4], int gradIndex, bool* marchedCubes)
{
...
TRIANGLE* MarchOneCube(int ncellsX, int ncellsY, int ncellsZ,
	...
	int ind, int i, int j, int k,
	float minValue, mp4vector * points, TRIANGLE *triangles, int &numTriangles,
	mp4vector verts[8], Point3 intVerts[12], int &edgeIndex,
	mp4vector gradVerts[8], Point3 grads[12], int &indGrad)
{
