...
public:
	...
	mp4vector() { x = 0, y = 0, z = 0, val = 0; }
	mp4vector(float aa, float bb, float cc, float dd) { x = aa; y = bb; z = cc; val = dd; }
	mp4vector(const mp4vector& other) { x = other.x; y = other.y; z = other.z; val = other.val; }
	mp4vector(const Point3& v, const float value) { x = v.data()[0]; y = v.data()[1]; z = v.data()[2]; val = value; }
	void operator = (const mp4vector& v) { x = v.x; y = v.y; z = v.z; val = v.val; }
	operator Point3() const { Point3 p(x, y, z); return p; }
};
...
//	ind: index of this cube in the points array (this is so it doesn't have to be computed again)
...
//	prevVerts: adjacent 4 vertices of the previous cube, passed in the special order which the corresponding
//		MCFace function recognizes. For specification on which indexes passed from the last cube go to 
...
//		that is specified in their names (for numbering see my web page). Each assigns the previous computed
